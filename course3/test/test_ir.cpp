...
TEST(test_ir, pnnx_graph_operands_and_params) {
    ...
    for (int i = 0; i < ops.size(); ++i) {
        ...
        LOG(INFO) << "OP Inputs";
        for (int j = 0; j < op->inputs.size(); ++j) {
            OG(INFO) << "Input name: " << op->inputs.at(j)->name
                    << " shape: " << ShapeStr(op->inputs.at(j)->shape);
        }
        ...
        for (const auto& weight : op->attrs) {
            LOG(INFO) << weight.first << " : " << ShapeStr(weight.second.shape) << " type "
                    << weight.second.type;
        }
        LOG(INFO) << "---------------------------------------------";
    }
...
TEST(test_ir, pnnx_graph_all_homework) {
    ...
    for (const auto& operator_ : operators) {
        ...
        ASSERT_EQ((dynamic_cast<RuntimeParameterBool*>(parameter_bool)->value), true);
        /////////////////////////////////
        ...
        ASSERT_EQ((dynamic_cast<RuntimeParameterInt*>(parameter_in_features)->value), 32);
        ...
        ASSERT_EQ((dynamic_cast<RuntimeParameterInt*>(parameter_out_features)->value), 128);
        }
    }
}